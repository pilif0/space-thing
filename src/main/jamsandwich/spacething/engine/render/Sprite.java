...
public class Sprite {
	...
	public Sprite rotate(double angle) {
		...
		for(int i = 0; i < vertices.length; i++) {
			...
			vertices[i][0] = c[0] + (distX*Math.cos(Math.toRadians(360-angle)) + (distY*Math.sin(Math.toRadians(360-angle))));
			vertices[i][1] = c[1] - (distX*Math.sin(Math.toRadians(360-angle)) - (distY*Math.cos(Math.toRadians(360-angle))));
		}
	...
	public void rotateAround(double[] pos, double angle) {
		for(int i = 0; i < vertices.length; i++) {
			...
			vertices[i][0] = pos[0] + (distX*Math.cos(Math.toRadians(360-angle)) + (distY*Math.sin(Math.toRadians(360-angle))));
			vertices[i][1] = pos[1] - (distX*Math.sin(Math.toRadians(360-angle)) - (distY*Math.cos(Math.toRadians(360-angle))));
		}
		center=centroid();
	}
